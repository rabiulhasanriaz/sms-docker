...
@section('general_sms_send_menu_class','active')
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            ...
            <table class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                @php($serial=1)
                @foreach($schedule_general_sms as $schedule_sms)
                    <tr>
                        <td>{{ $serial++ }}</td>
                        <td title="{{ $schedule_sms->sci_campaign_id }}">{{ $schedule_sms->sci_campaign_title }}</td>
                        <td>{{ \Carbon\Carbon::parse($schedule_sms->created_at)->format('H:i a, d-M-Y') }}</td>
